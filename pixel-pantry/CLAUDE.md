...
E-shopový projekt v rámci kurzu Claude Code. Obecný kontext uživatele
a přehled projektů viz kořenový `../CLAUDE.md`.
...
- **CI/CD** (GitHub Actions, štíhlá dvoustupňová varianta — detaily
  ...
  v projektu), s `working-directory: pixel-pantry/workers` a path-filtrem
  `pixel-pantry/workers/**`, aby změny v Django `backend/` (ani v jiných
  projektech monorepa) deploy Workeru nespouštěly. `ci.yml` = typecheck
