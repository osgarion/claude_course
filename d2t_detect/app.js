...
// Returns { p_d2t, p_rem, imputed: [names] }.
function predict(model, raw) {
  ...
  let eta = model.coefficients.raw_scale_intercept;
  const imputed = [];
  ...
  return { p_d2t, p_rem: 1 - p_d2t, imputed };
}
...
function buildForm(model) {
  ...
  for (const s of model.input_schema) {
    ...
    field.innerHTML = `
      <span class="lbl">${s.label} <span class="unit">${s.unit}</span></span>
      <input id="${id}" name="${s.name}" type="number" step="any"
             ${nonNeg ? 'min="0"' : ""}
             placeholder="median ${fmt(s.imputation_median, 2)}"
             aria-label="${s.label} in ${s.unit}" />
      <span class="range">observed ${fmt(s.observed_min, 2)}–${fmt(s.observed_max, 2)}</span>`;
...
function readForm(model) {
  ...
  for (const s of model.input_schema) {
    ...
    raw[s.name] = v;
    if (v < s.observed_min || v > s.observed_max) outOfRange.push(s.label);
  }
  return { raw, outOfRange };
}

function showResult(model, raw, outOfRange) {
  const { p_d2t, p_rem, imputed } = predict(model, raw);
  ...
  if (imputed.length) {
    ...
    const labels = imputed.map((n) => model.input_schema.find((s) => s.name === n).label);
