// ---------- Košík (localStorage, sdílený mezi stránkami) ----------
...
/**
 * Volání API. Oproti Django verzi tady není žádný CSRF token - autentizace
 * jde přes hlavičku Authorization, ne přes cookie, takže CSRF nehrozí.
 */
...
// ---------- Detail produktu (product.html na /produkt/<slug>/) ----------

function initProductDetail() {
  const root = document.getElementById("product-detail");
  // Slug čteme z cesty - Worker přepisuje /produkt/<slug>/ na tenhle soubor.
  ...
  function renderReviews() {
    if (product.reviews.length === 0) return "<p>Zatím žádné recenze.</p>";
  ...
  function renderReviewFormOrLoginPrompt() {
    ...
    return `<p><a href="/prihlaseni">Přihlas se</a>, abys mohl/a přidat recenzi.</p>`;
  }

  function renderProduct() {
    const mainImage =
      (product.images[0] && product.images[0].image) || product.image_url || placeholderImage;
        ...
        apiFetch(`/api/products/${product.id}/reviews`, {
          ...
          .catch(() => {
            messageEl.textContent =
              "Recenzi se nepodařilo uložit (možná už jsi tento produkt hodnotil/a).";
          });
      });
    }
  }
...
function initCheckout() {
  ...
  // Host adresu nikam neukládá předem - jde rovnou do objednávky jako
  // shipping_address_input. Přihlášený vybírá/zakládá uloženou adresu.
  if (!loggedIn) {
  ...
  if (loggedIn) {
    addressForm.addEventListener("submit", (event) => {
      ...
      apiFetch("/api/addresses/", {
        method: "POST",
        body: JSON.stringify(Object.fromEntries(new FormData(addressForm).entries())),
      })
        .then((response) => response.json())
        .then(() => {
          addressForm.reset();
  ...
  // Host se ke své objednávce prokazuje tokenem, který dostal při založení.
  const orderUrl = (path) => (guestToken ? `${path}?token=${guestToken}` : path);
  ...
  function renderPostOrderActions(order) {
    ...
    document.getElementById("pay-btn").addEventListener("click", () => {
      apiFetch(orderUrl(`/api/orders/${order.id}/pay`), { method: "POST", body: "{}" })
        .then((r) => r.json())
        .then(() => {
    ...
    document.getElementById("cancel-btn").addEventListener("click", () => {
      apiFetch(orderUrl(`/api/orders/${order.id}/cancel`), { method: "POST", body: "{}" })
        .then((r) => r.json())
        .then(() => {
          messageEl.textContent = "Objednávka zrušena.";
        });
    });
  }

  placeOrderBtn.addEventListener("click", () => {
    if (loggedIn && !selectedAddressId) {
      messageEl.textContent = "Vyber nebo přidej doručovací adresu.";
      return;
    }
    if (!loggedIn && !addressForm.reportValidity()) return;

    apiFetch("/api/orders/", { method: "POST", body: JSON.stringify(orderPayload()) })
      .then(async (response) => {
        const data = await response.json();
        if (!response.ok) throw data;
      ...
      .catch((error) => {
        messageEl.textContent =
          error && error.detail ? error.detail : "Objednávku se nepodařilo vytvořit.";
      });
  });
